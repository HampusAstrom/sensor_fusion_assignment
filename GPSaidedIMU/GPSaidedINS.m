...
for k=2:N
    ...
    y = [in_data.GNSS.pos_ned(:,ctr_gnss_data); in_data.SPEEDOMETER.speed(:, ctr_speed_data); in_data.GNSS.HDOP(:,ctr_gnss_data); in_data.GNSS.VDOP(:,ctr_gnss_data);];
...
%---> Hint:  Rn2p = get_Rb2p()*q2dcm(x_h(7:10))';
    ...
    H = [eye(3) zeros(3,12); zeros(3, 3) Rn2p zeros(3, 9)];
    
   
    
    %R = settings.sigma_gps^2*eye(3)
    R = diag([settings.sigma_gps^2 settings.sigma_gps^2 settings.sigma_gps^2 settings.sigma_speed^2 settings.sigma_non_holonomic_h^2 settings.sigma_non_holonomic_v^2]);
